...
class EncoderLayer(nn.Module):
    def __init__(self, self_attention, d_model, d_core, n_heads, seq_len, d_ff=None, dropout=0.1, activation="relu"):
        super(EncoderLayer, self).__init__()
        d_ff = d_ff or 4 * d_model
        self.self_attention = self_attention
        self.temp_gate = nn.Linear(2 * d_model, 1)
    ...
    def forward(self, x, B, N, D, cycle, var, x_mask=None, tau=None, delta=None):
        x = x + self.dropout1(self.self_attention(
            x, x, x,
            attn_mask=x_mask,
            tau=tau, delta=None
        )[0])
        ...
        combined_mean = torch.cat([proj, x_glb], -1)
        core = F.gelu(self.gen4(F.gelu(combined_mean)))
        core = self.core_extract(core)
        ...
        # mlp fusion
        combined_glb_cat = torch.cat([combined_mean, core], -1)
        ...
        y = self.dropout2(self.conv2(y).transpose(-1, 1))
...
class Encoder(nn.Module):
    ...
    def forward(self, x, b, n, d, cycle, var, x_mask=None, tau=None, delta=None):
        for layer in self.layers:
            x = layer(x, b, n, d, cycle, var, x_mask=x_mask, tau=tau, delta=delta)
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, cycle_index, x_mark_enc, x_dec, x_mark_dec):
        ...
        enc_out = self.encoder(enc_out, b, n, d, cycle, var_embed)
